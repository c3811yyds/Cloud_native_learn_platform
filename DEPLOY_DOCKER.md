...
4. Redis 服务
使用 `redis:7-alpine`，容器名固定为 `cnlp-redis`，当前用于：
...
DATABASE_URL=mysql+pymysql://admin:admin@db:3306/cloud_native_learning_platform?charset=utf8mb4
...
docker compose exec -T db mysqldump -u root -proot --default-character-set=utf8mb4 --no-tablespaces cloud_native_learning_platform > /root/db-backups/backup_$(date +%F_%H%M).sql
```
...
```bash
docker compose exec -T db mysql -u root -proot cloud_native_learning_platform < db-backups/database_seed.sql

tar -czf /root/backend_storage_sync.tar.gz -C /root/cloud-native-learning-platform backend/storage
...
```bash
docker compose exec -T db mysqldump -u root -proot --default-character-set=utf8mb4 --no-tablespaces cloud_native_learning_platform > /root/db-backups/server_dump_$(date +%F_%H%M).sql
```
...
```bash
mysql -u root -p cloud_native_learning_platform < ./db-backups/server_dump_时间.sql
```
...
十二、GitHub Actions 手动部署

当前仓库已提供手动触发的部署工作流：

- workflow 文件：`.github/workflows/deploy.yml`
- 触发方式：GitHub 仓库 `Actions -> Deploy -> Run workflow`
- 默认部署分支：`main`

使用前需要在仓库 `Settings -> Secrets and variables -> Actions` 中配置这些 Secrets：

- `DEPLOY_HOST`：服务器公网 IP 或域名
- `DEPLOY_PORT`：SSH 端口，通常是 `22`
- `DEPLOY_USER`：部署用户
- `DEPLOY_PATH`：服务器上的项目目录，例如 `/cnlp/Cloud_native_learn_platform`
- `DEPLOY_SSH_PRIVATE_KEY`：可登录服务器的私钥

工作流执行内容：

1. 使用 SSH 连接服务器
2. 进入 `DEPLOY_PATH`
3. 执行 `git fetch origin`
4. 切换到目标分支/引用并 `git pull --ff-only`
5. 执行 `docker compose up -d --build`
6. 用 `curl http://127.0.0.1:5000/api/health` 做一次后端健康检查

说明：

1. 这是手动部署，不会在每次 push 后自动上线。
2. 服务器工作区如果有未提交改动，`git pull --ff-only` 可能失败；部署前请保持服务器代码目录干净。
3. 建议先让 CI 通过，再手动点部署。
## 端口收口说明（v1.9.7）
...
```bash
cd ~/cloud-native-learning-platform
mysql -u admin -p -h 127.0.0.1 -P 3306 cloud_native_learning_platform
```
