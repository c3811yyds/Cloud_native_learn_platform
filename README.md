...
1. 项目定位：云原生学习平台，用于学习和演练前后端分离、Docker 部署与工程化实践。
...
2. 后端运行 (Flask)
   ...
   - (2) 创建并选择数据库
     - CREATE DATABASE cloud_native_learning_platform DEFAULT CHARACTER SET utf8mb4;
   ...
   - (7) 配置环境变量
     ...
     - REDIS_URL=redis://127.0.0.1:6380/0 (本地建议连接独立的 cnlp-redis 容器；线上接入 Redis 服务后改成对应容器地址)
